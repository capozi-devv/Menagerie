...
public class AllVFX {
    public static void renderSolidPurpleCube(MatrixStack matrices, VertexConsumerProvider vertexConsumers, BlockPos position, float size) {
        ...
        drawQuad(vertexConsumer, matrix, normalMatrix, minX, minY, minZ, maxX, minY, minZ, maxX, maxY, minZ, minX, maxY, minZ, r, g, b, a);
    }

    private static void drawQuad(VertexConsumer consumer, Matrix4f matrix, Matrix3f normalMatrix, float x1, float y1, float z1, float x2, float y2, float z2, float x3, float y3, float z3, float x4, float y4, float z4, float r, float g, float b, float a) {
    ...
    private static RenderTypeToken getRenderTypeToken() {
        return RenderTypeToken.createToken(new Identifier(Menagerie.MOD_ID, "textures/entity/beam4.png"));
    }
    ...
    public static void renderObelisk(MatrixStack matrixStack, Vec3d pos) {
        ...
        if (isFlashing()) {
            builder.replaceBufferSource(RenderHandler.DELAYED_RENDER.getTarget())
                    ...
                    .setAlpha(0.9f)
                    .setLight(200)
                    .renderBeam(matrix4f, pos.add(0.5, 0, 0.5), pos.add(0.5,1000,0.5), 15);
        }
        matrixStack.pop();
    }
}
